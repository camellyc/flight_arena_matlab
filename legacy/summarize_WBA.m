...
% Plot wingbeat amplitude percentage change data, before, during, and after optogenetic
...
final_summary_file_name = strcat(summary_title, '_WBA');
 
% Plot settings
x_limits = [-2, 10];    % X-axis limits in seconds (before and after LED ON)
y_limits = [-75, 25];     % Y-axis limits for WBA percentage plots
trace_color = [0, 0.5, 1]; % Color of the WBA traces (blue with specified RGB values)
...
smooth_window = 1000;      % Smoothing window size (in samples)
...
y_title = '\DeltaWBA (%)';   % Y-axis label for percentage change
...
for fileIdx = 1:length(files)
    ...
    for j = 1:length(uniqueStimDurations)
        ...
        for block = 1:blocks
            ...
            for i = 1:length(indices_for_duration)
                ...
                if end_idx > length(Data(dr.wbaL,:)), end_idx = length(Data(dr.wbaL,:)); end
 
                % Extract and calculate WBA data (FIXED: using Data(dr.wbaL,:) and Data(dr.wbaR,:))
                wba_segment = (Data(dr.wbaL, start_idx:end_idx) + Data(dr.wbaR, start_idx:end_idx))/2; % WBA data
                smoothed_wba = smoothdata(wba_segment, 'movmean', smooth_window); % Smoothing
 
                % Calculate baseline WBA (mean WBA 2 sec before LED ON)
                baseline_wba = mean(smoothed_wba(1:min(pre_samples, length(smoothed_wba))));
                
                % Convert to percentage change: ((current - baseline) / baseline) * 100
                % Handle case where baseline is zero to avoid division by zero
                if baseline_wba == 0
                    warning('Baseline WBA is zero for file %s, stimulus %d. Setting percentage change to 0.', files{fileIdx}, i);
                    percentage_wba = zeros(size(smoothed_wba));
                else
                    percentage_wba = ((smoothed_wba - baseline_wba) / baseline_wba) * 100;
                end
                ...
                % Store the percentage change segment in the matrix
                block_wba_segments = [block_wba_segments; percentage_wba];
            end
            
            grouped_block_traces{j, block} = block_wba_segments;
            grouped_wba_segments{j} = [grouped_wba_segments{j}; block_wba_segments];
        end
    end
 
    % Step 4: Plot the individual data
    fileSummary = struct(); % To hold the average WBA for each duration in this file
    ...
    for j = 1:length(uniqueStimDurations)
        ...
        if ~isempty(all_wba_segments)
            % Calculate the mean WBA trace across all blocks
            mean_wba = nanmean(all_wba_segments, 1); % Use nanmean to ignore NaNs
 
            % Store the mean_wba data for summary plot
            fileSummary.(sprintf('stim_%d', currentStimDuration)) = mean_wba;
 
            % Create time axis for plotting
            time_axis = linspace(x_limits(1), x_limits(2), length(mean_wba));
            ...
            for block = 1:blocks
                block_wba = grouped_block_traces{j, block};
                if ~isempty(block_wba)
                    block_mean_wba = nanmean(block_wba, 1); % Use nanmean to ignore NaNs
                    plot(time_axis, block_mean_wba, 'LineWidth', 1, 'Color', [trace_color, trace_opacity]);
                end
            end
 
            % Plot the mean WBA trace across all blocks with thicker line
            plot(time_axis, mean_wba, 'LineWidth', 2, 'Color', trace_color); % Solid blue line
            ...
            file_name = [files{fileIdx}(1:end-4), '_WBA_plot_', num2str(currentStimDuration), 'ms'];
...
for j = 1:length(uniqueStimDurations)
    ...
    figure;
    hold on;
    time_axis = linspace(x_limits(1), x_limits(2), length(mean_wba)); % Assuming all files have the same time axis length
    ...
    for fileIdx = 1:length(summary_data)
        ...
        if isfield(file_data, sprintf('stim_%d', currentStimDuration))
            file_wba = file_data.(sprintf('stim_%d', currentStimDuration));
            plot(time_axis, file_wba, 'LineWidth', 1, 'Color', [trace_color, trace_opacity]); % Individual files with lighter lines
        end
    end
...
% Set y-axis limits and ticks for percentage
ylim(y_limits);
yticks(-50:25:50); % Set y-ticks every 25% for percentage scale
