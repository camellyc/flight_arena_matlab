...
%
% 2026-02-19 MOD (Phantom external F-Sync + PCC pretrigger workflow):
%   - Start Phantom CAPTURE early (like clicking "Capture" in PCC)
%   - Do NOT force dFrameRate when external sync
%   - Do NOT set PostTriggerFrames/PTFrames in MATLAB (PCC controls pretrigger)
%   - Fire HW trigger at end of window (t = window_s(2)) to STORE cine
%   - Increase trigger pulse width (50 ms) for robustness
%   - Increase store wait timeout
%   - fclose(fid1) to avoid truncation
%
% 2026-03-05 MOD:
%   - Change Phantom save output from .cine to TIFF 12 image sequence
%   - Save TIFF sequence into a folder named exactly like the old cine basename
...
saveFolder = ['H:\.shortcut-targets-by-id\10pxdlRXtzFB-abwDGi0jOGOFFNm3pmFK\Tuthill Lab Shared\Yichen\', ...
                    'Spiracle\Flight_Arena_Data\260429_empty_A1ACR\'];
...
experiment_name = 'empty_A1ACR';     % Example: spSN_ChR
genotype = 'empty_A1ACR_4d_F';      % Example: SS48339_2d_M
flyNumber = '6';                    % Example: Fly1
trialNum = '4';
stimulus_regime = '0ms';  % Example: 0-3000ms
stimulus_position = 'thorax';
phantom_position = 'sp1'; % Example: sp1, sp2, wing
visual_stim_type = 'oscillating_sine_wave_20';
carbon_dioxide = 'ON';

FlyType = [experiment_name '_' genotype '_Fly' flyNumber '_Trial' trialNum '_' stimulus_regime '_' stimulus_position '_' phantom_position '_' visual_stim_type '_' carbon_dioxide];
...
R_X_Pos = L_X_Pos;
variables.blocks = 1;
variables.TrialLength = 100;

% LED display pattern variables
variables.velfunc_id = 5; %function_sine_0.05hz.mat
variables.y_gain = 20; % equals to pixels/s
variables.y_bias = 0;
...
% USER-DEFINED OPTOGENETIC STIMULUS WINDOW (seconds)
stimStartTime_s = 91;   % stimulus starts at 10 s
stimEndTime_s   = 91.5;   % stimulus ends at 55 s

% NI + Basler
variables.SampleRate = 20000;
variables.Basler_fps = 100;
variables.Exposure_time = 9000;
...
%% ---------- CONSISTENT BASENAME FOR ALL OUTPUTS ----------
now = datetime('now','TimeZone','local');
formatOut = 'yyyy_mmdd_HHMMSS';
savedate = datestr(now, formatOut);                 % char
baseFileName = [savedate '_' char(FlyType)];        % char, same base as .mat (via save)

% Binary log file: EXACT same base, no extension (keeps your "latest no extension" logic)
FILENAME = fullfile(saveFolder, baseFileName);
fid1 = fopen(FILENAME,'w');
if fid1 == -1
    error('Could not open binary log file for writing: %s', FILENAME);
end
%% --------------------------------------------------------
...
% PCC workflow: CAPTURE first, trigger at 90 s, keep 80 s pre-trigger
phantom.window_s = [10 95];              % window start, window end (80 s duration)
phantom.arm_lead_s = 0.5;
...
phantom.gate_line = 'Port0/Line7';
phantom.gate_poll_period_s = 0.001;
...
%% Initialize NI device and sessions
...
% AI 4/data(6,:) : EMG recording OR Converted Hutchen Trigger Input split
...
if phantom.enable
    try
        ...
        % DO NOT set dFrameRate in external F-Sync mode (PCC controls sync)
        % phantom.aqParams = setIfFieldExists(phantom.aqParams, 'dFrameRate', double(phantom.fps));

        % Exposure only (safe to set)
        ...
        % DO NOT override PTFrames/PostTriggerFrames/PTFrames here.
        % PCC is already set for "80 s pretrigger before trigger".
...
%% Initialize storage for stimulus orders
...
for block = 1:variables.blocks
    ...
    randomizedStimOrder = stimDurations;
    allRandomizedStimOrders{block} = randomizedStimOrder;

    outputSignal = zeros((variables.TrialLength * variables.SampleRate), 1);

    % Safety checks for user-defined stimulus window
    if stimStartTime_s < 0 || stimEndTime_s > variables.TrialLength || stimEndTime_s <= stimStartTime_s
        error('Invalid stimulus window: make sure 0 <= stimStartTime_s < stimEndTime_s <= TrialLength.');
    end

    for i = 1:length(randomizedStimOrder)
        currentStimDuration = randomizedStimOrder(i);

        % Ignore currentStimDuration for placement; use explicit start/end window
        nPulseDuration = round(variables.SampleRate * (variables.PulseDuration / 1000));
        nIPI = round(((1000 / variables.Frequency) - variables.PulseDuration) * variables.SampleRate / 1000);

        stimStartIdx = round(stimStartTime_s * variables.SampleRate) + 1;
        stimEndIdx   = round(stimEndTime_s   * variables.SampleRate);

        for o = 0:(nPulseDuration - 1)
            pulseIdx = stimStartIdx + o : (nPulseDuration + nIPI) : stimEndIdx;
            pulseIdx = pulseIdx(pulseIdx >= 1 & pulseIdx <= length(outputSignal));
            outputSignal(pulseIdx, 1) = 10;
        end
    end

    % -------------------------
    % Build Phantom trigger waveform on AO0 (Trigger at END time to STORE cine)
    % -------------------------
    ...
    if doPhantomThisBlock
        trig_t_s = phantom.window_s(2);  % trigger at 90s (store)
    ...
    % Queue AO: [AO1 LED, AO0 Phantom trigger]
    queueOutputData(mainSession, [outputSignal, phantomSignal]);

    tic
    Panel_com('set_pattern_id', 2); pause(panel_pause);
    ...
    % -------------------------
    % Start Phantom CAPTURE early + poll Recording gate
    % -------------------------
    tSess0 = tic;
    armed = false;

    % Start capture shortly before window start (mimic clicking PCC "Capture")
    ...
    % -------------------------
    % Save Phantom TIFF12 sequence for this block
    % -------------------------
    if doPhantomThisBlock
        try
            disp('Waiting for Phantom cine to store...');
            waitForStoreOrTimeout(phantom.CN, phantom.camObj, 180);
...
% Set arena closed loop after trials
...
% Remove listeners and CLOSE binary log file
delete(lh);
delete(lh2);
try
    fclose(fid1);
...
%% Read the most recent binary file without extension (your existing logic)
d = dir(saveFolder);
isFile = ~[d.isdir];
d = d(isFile);
...
% ---- Consistent .mat base name: SAME baseFileName as everything else ----
...
function waitForStoreOrTimeout(CN, camObj, timeout_s)
    ...
    while toc(t0) < timeout_s
        ...
        % Prefer camObj status if available
        try
